...
| Is the problem evidence, currentness, provenance, credential-status, generated-source relation, copied-source relation, or source-chain recovery? | `A.10` | A descriptive claim-bound evidence-provenance account, its cited direct relations and currentness facts, and its RelianceDisposition for the attempted use. |
| Is an actual named assurance claim required for this use? | `B.3` | The separately governed assurance result with its target, basis and limits; use the direct subject pattern when no assurance claim is current. |
