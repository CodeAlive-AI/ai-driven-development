...
Use the `Preface` for the cross-cutting ideas and the repeated-use explanation. Use the Table of Contents when you already know the pattern family or need a search-oriented overview. Use the direct pattern body for the governed Solution. Once one direct pattern is current, use `E.11.PUA` to follow its Solution: identify the smallest independently governed result usable now, or stop and name the missing direct basis. Name a receiving use only when an actual continuation or later reliance is current. Use `E.11.PUR` when applicability, recommendation, coordination, or ordering among candidate pattern uses is the current question. An ordinary reversible judgement may remain conversational; make it addressable only when a named later use needs that support. Use extended cases when the compact card and direct pattern are not enough.
