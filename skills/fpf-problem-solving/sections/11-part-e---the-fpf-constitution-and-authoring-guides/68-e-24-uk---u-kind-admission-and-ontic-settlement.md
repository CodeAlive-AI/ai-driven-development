...
| Is additional durable ontic coordination needed? | `Outputs.OntologyDispositionResult` records direct use, bounded coordination or a genuinely needed durable ontic. `U-kind-only` may cite a needed accepted ontic or explicitly conclude that no additional ontic is needed; `atomic ontic-plus-U-kind` creates sibling outputs only when both are independently needed. |
...
7. **Shared E.24-family settlement.** Fill `E.24:4.0a` with the subject kind and identity rule, the smallest governed relation set needed by the named use, any identity-bearing relation selected by the current settlement decision, declarations actually reused, direct defining or testing rules, receiving use, and non-use and reopen boundaries. Also cite the durable-membership rule and scheme, the same-individual inclusion law or identity-dependence relation when applicable, and the exact result references. Record whether an additional durable ontic is actually needed. If direct rules suffice, return the direct-use or bounded-coordination disposition and no new ontic result. Only when both new outputs are independently needed does an atomic co-decision return separate provisional outputs without circular premises.
...
- Identity dependence concerns two distinct individuals joined by a governed relation that contributes to one individual's identity. A proposed capability individual distinct from its holder would need that relation and a receiving use that requires its continuity. A.2.2 instead states qualified ability about the holder; a holder reference in that assertion supplies no second individual.
...
The recovered A.2.2 capability need takes the `reject` exit in section 4.2c: ordinary holder-ability claims remain usable without a second kind or an invented dependence relation.
...
| `E24UK-AR-UKIND-R5-01` | `U.Kind`; `root`, R5 | `C.3:4` and `C.3.1:4–6`; one reusable intensional classification distinction recovered through candidate domain, operative membership condition, intended member/non-member boundary and continuity rule; section 4.2.2 retains this admission independently of the additional-ontic question | C.3 typed reasoning and durable kind settlement; a `KindSignature`, locality, scheme, current extension, label, assertion or publication is not the kind individual |
...
| `E24UK-NAR-U-CAPABILITY-01` | `U.Capability`; `reject` | A.2.2 planning, promise, fit and development uses require a holder System and qualified ability claims, not another individual or classificatory distinction. Use `U.System` and C.2.1 epistemes when an assertion needs an identified account. |
| `E24UK-NAR-LSMT-01` | `U.LanguageStateMoveTrajectory`; `reject` | A.16.0 needs an ordinary C.2.1 history account and an E.24.PUB expression/publication form. The recovered branch, loss, lineage and responsibility uses require no additional individual or durable classification. |
| `E24UK-NAR-AIPR-01` | `U.ActionInvitationPrecisionRestoration`; `reject` | A.6.A governs a precision-restoration move; the title admits no further kind, and any exact relation claim still needs its direct obtaining predicate and identity rule |
...
The root membership result is sufficient for recognizing such kind individuals. It does not require another durable ontology unit. C.3.1 comparison independently uses exact kind identities and the `U.SubkindOf` predicate; C.3.2 classification independently uses its admissibility and membership rules. Those receiving uses cite their direct rules without deriving them from an empty relation set. If several already defined claims must be read together for a bounded task, a C.2.1 episteme may coordinate them without becoming a durable ontic.

The ontology-disposition decision `E24-CO-UKIND-R19-01` selects `subject-pattern-use` for these named uses and concludes that no additional durable ontic is needed. Its decision and result differ from the originating R5 decision; the root admission and direct identity/membership law remain the same. The exact new decision and unchanged R5 provenance are preserved in the [U.Kind settlement decision](../_change-campaigns/repo-front-door/PATTERN-TITLE-DISCOVERABILITY-AND-SELECTION-CAMPAIGN/UKIND-ONTIC-SETTLEMENT-DRR.md).

For example, C.3.1 can establish `CoolingPumpKind` as a subkind of `PumpKind` when the aligned cooling-pump criterion entails the pump criterion. An extension sample alone cannot do so. C.3.2 can classify an admissible Pump #14 under its actual declaration and slice; a missing required cooling-performance fact yields `unknown`, while an out-of-domain request is `not-applicable` before classification. Neither result needs a new ontology unit. A project-specific kind remains an individual of `U.Kind` without gaining its own durable public `U.*` name.
...
5. **Choose the positive form.** Use `root` for independently identified individuals, `same-individual-dependent` when one root individual gains an additional stable membership predicate and inclusion law, or `identity-dependent` when a distinct individual has an already governed dependence relation to one root individual plus all discriminators. Fill the shared E.24-family decision and separately test the need for additional durable coordination. Direct-rule sufficiency permits positive kind admission with an explicit no-additional-ontic conclusion; use atomic co-decision only when both new outputs are needed. Apply A.11 and A.8 when kernel status is claimed.
6. **Close or leave unresolved, then name.** Record the positive admission when the selected form passes every condition. A missing branch law or fact leaves admission unresolved. Use `reject` only when no classificatory distinction survives, and recover the non-kind object under the rule that defines or tests it. Only after one disposition and governed object are stable may F.8, F.5, F.18, or F.17 expose a public name.
...
| public durable FPF kind needed across patterns | use E.24.UK with the shared E.24:4.0a decision; admit the kind on its direct membership/identity grounds and record whether a needed ontic is reused, both new outputs are independently needed, or direct rules suffice without an additional durable ontic |
...
| `CC-E24UK-2b` | `E24UK-AR-UKIND-R5-01` retains U.Kind root admission through its C.3/C.3.1 candidate domain, operative membership condition, member/non-member boundary and continuity rule. E24-CO-UKIND-R19-01 independently returns direct subject-pattern use without an additional durable ontic; the original R5 decision remains provenance. Locality, spelling, scheme, declaration, extension, assertion and publication decide neither identity nor membership by themselves. |
...
| `CC-E24UK-3` | Every positive result cites the kind's direct membership, extent and branch-specific law and all eight admission conditions. Its shared decision either cites an accepted ontic actually needed, supplies a separately justified new ontic, or explicitly records direct use or bounded coordination with no additional durable ontic. A reconstructed RG result keeps its exact admission reference and by-value basis; test any newly needed coordination instead of inventing a sibling identifier. |
...
| `CC-E24UK-3c` | The practitioner-first admission tree tests exact existing-kind coverage and bounded C.3 classification before new admission; a resolved non-admission closes as `reuse` with its accepted result, `local-kind` with one exact C.3.2 declaration, or `reject` with the actual object and direct governor recovered under section 4.6. A missing required law or fact leaves admission unresolved rather than supplying one of those results. |
| `CC-E24UK-3d` | Only when both a new ontic and a new public kind are independently needed does an atomic E24FamilySettlementDecision return separate results from common inputs, with neither used as prior evidence and both provisional while a required branch is unresolved. A justified no-additional-ontic conclusion uses U-kind-only and the existing direct-use or bounded-coordination disposition; it never admits an empty connected unit. |
...
| `CC-E24UK-4c` | Every retained public example resolves through one exact E24UK-AR-* admission-result reference to its direct membership/identity basis and decision provenance. Follow an ontic output only when the receiving use actually consumes it. The U.Kind R5 root result is preserved while its additional-ontic disposition is stated separately; a reconstructed result retains its cited direct passage and by-value basis. Labels, row shape and unresolved decisions establish no admission. |
...
| --- | --- | --- | --- |
| [gUFO, March 2026](https://arxiv.org/abs/2603.20948), with its type typology, intrinsic and relational aspects, situations, and higher-order types; [ISO/IEC DIS 21838-5](https://www.iso.org/standard/89915.html) is a draft under development, not a published standard (ISO status observed 2026-09-25) | Rich distinctions expose whether one bearer gains a classification or a distinct dependent individual needs grounding. | Using the whole typology first adds a foundational-ontology mapping and can choose an imported category before the FPF receiving use and local alternative are known. | **Adapt** the same-individual/distinct-individual question in the admission tree and the WorkPlan, capability, role, phase, and quality cases. **Reject** automatic import of the taxonomy or its labels as FPF admission. |
| Current [BFO 2020 artifacts](https://github.com/BFO-ontology/BFO-2020), maintained for the ISO 21838 top-level-ontology line | Strong formal separation of dependence, continuant mereology, process, role, and disposition. | Adopting the whole upper ontology is expensive for a bounded FPF authoring decision, and its categories do not decide whether one FPF name deserves durable public reliance. | **Adopt** the warning that dependence is not parthood and require an exact dependence rule when a distinct dependent individual is actually needed. **Reject** BFO classification or official status as the admission gate. |
...
- **Shares settlement with:** `E.24` through the one `E24FamilySettlementDecision` schema in `E.24:4.0a`. E.24.UK records the `UKindAdmissionResult`; E.24 records the ontology disposition and an `OnticSettlementResult` only when a durable ontic is selected. An existing result may be reused, while a case needing both new outputs is one atomic co-decision with neither output used as prior evidence.
