...
* **No-mechanism equivalence:** MVPK is not a mechanism and no face acts. Build, rendering, upload, or delivery may be actual `U.Work` performed by a capable System. When such Work is current, A.15.1 identifies the Work, performers, Method, time, and containing System; F.6 identifies the assignment under which each performer acted. A short publication account may omit assignment or Method identifiers that its bounded use does not need. Name a carrier relation separately only when the claim or downstream use depends on it.
