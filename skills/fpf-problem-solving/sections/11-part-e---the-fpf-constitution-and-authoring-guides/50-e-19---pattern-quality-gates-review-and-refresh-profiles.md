...
For one actual E.19 review or refresh, keep independently recoverable:
...
3. a separately identified semantic review `U.Method`;
4. the reviewer `U.System`, obtaining `U.RoleAssignment`, dated review `U.Work`, and enacted method;
5. each exact PCP check application and A.6.1 binding to the reviewed loci;
...
9. witnesses and exact A.10 evidence-use/provenance relations, plus any B.3 assurance or reliance result; and
10. any F.10 status value/use/interpretation, publication occurrence/form/carrier, and currentness relation.
...
Name the exact reviewed pattern or landing subset, its edition or other stable version basis, the admission or refresh question, the baseline and risk-selected profile questions in scope, and the exact review configuration. Profile choice narrows review depth; it does not turn the selected questions into a progress record or perform the review.

When actual review is claimed, identify the semantic review method, reviewer system and obtaining role assignment, dated review work, and exact PCP check applications/bindings. If a durable aggregate result is needed, constitute a C.2.1 result episteme whose EntityOfConcern is the exact reviewed pattern edition or subset and whose ClaimGraph states the review scope, applicable profile questions, actionable findings/blockers or aggregate cleared boundary, conclusion, non-use, and reopen condition. Witnesses, evidence use, the optional result record/publication, and any authority-bearing admission or refresh decision remain separate.
**Choose one review form.** An `E.19` review has two forms:

1. **Inspect, repair, and verify.** One bounded portion may coordinate dated review work, authoring/repair work, and focused-verification work; identify them separately whenever their performer, method, affected object, or result differs. Review work applies every selected question, repair work changes each in-scope defect in the pattern/subset, and verification work reapplies the affected checks. The changed edition and focused-verification claims carry the substantive evidence; constitute an aggregate E.19 result episteme only when a receiving admission/refresh decision requires it. A separate findings record is made only for an unresolved blocker, a decision outside current authority, or transfer to another author.
2. **Independent findings.** Dated reviewer work applies the selected questions but does not change the reviewed pattern/subset. One C.2.1 findings-result episteme or semantic handoff records every actionable defect and blocker, with repair direction precise enough for the author to act without repeating the diagnosis. It is neither review work nor an admission decision.
...
* **Precision-restoration distribution must be preserved.** When an `E.10` scan selects a non-local precision-restoration path, the run checks that `E.10` remains the trigger and applicability description, `E.10.ARCH` states the shared recovery architecture, the selected realization pattern supplies the concrete ontological-unpacking rules for its locus (`A.6.P`, `C.2.P`, `C.30.P`, `C.16.P`, `C.16.Q`, `A.19.SPR`, or another selected restoration pattern), and affected patterns keep thin declarative pointers rather than local trigger registries or duplicate recovery algorithms. Treat a realization pattern as a `U.MethodDescription`, and identify a `U.Method`, performer, assignment, or `U.Work`, only when its actual `Solution` describes a method and that distinction changes the claim.
* **EntityOfConcern and precision-restoration questions travel with the same triage.** When the reviewed change touches EntityOfConcern, same-referent, slot/reference, alignment-path, role-boundary, consumer-disposition wording, description/publication-use guards, phrase apparatus, repeated boundary doctrine, architecture-placement rationale, package-boundary rationale, or quality/projection evidence, the run asks before acceptance: what is the pattern's own `EntityOfConcern` and first useful move; does the text state its own subject kind, action guidance or ordered first-use actions, practical delta, and bounded non-use before auxiliary material; which concrete pattern content defines, constrains, or tests each outside claim, relation, or boundary, and is an exact predicate or `ClaimGraph` needed to preserve truth, distinguish a nearby value, or support named reliance; does `F.19` phrase-restoration guidance apply before word/head/use restoration; which concrete contribution from `E.10`, `E.10.ARCH`, `F.18`, or another selected pattern should guide the remaining repair; do actual role, method, work, evidence, assurance, gate, status, and decision assertions retain the predicates and defining or constraining sources they need; and has every current-host consumer of the selected-family repair received a semantic, mechanical, compatibility, or not-triggered disposition. Treat cited pattern content as a `U.MethodDescription` or admitted `U.Method` only when its actual `Solution` describes a method and that distinction changes the claim. When E.21 is active, its separate result episteme contains these quality-effect assertions through `PrecisionRestorationProfile`; E.19 does not duplicate them as local rows.
...
| --- | --- | --- |
| **CC-E19-0 (Review specification, work, result, and decision remain distinct).** | Keep the exact reviewed pattern edition/subset, PCP declarations and review configuration, semantic review method, reviewer assignment, dated review work, check applications, repair/verification work, finding and result epistemes, witnesses/evidence use, optional record/publication, admission or refresh decision, assurance, status use, and currentness independently recoverable. A profile, checklist, record, or result performs no work and grants no decision. | Prevents editorial declarations and records from acting as reviewers or authority. |
...
| **CC-E19-1 (Baseline triage is mandatory).** | Every configured PQG review **MUST** include **PCP-BASE** for the reviewed pattern or subset. When dated review work completes the baseline and finds no risk requiring another profile, the review may finish after any small mechanical defect is repaired and focused verification or returned as an independent finding. This is only an `E.19` review boundary; it cannot support an `E.21` coordinate value, `PatternQualityStatus`, all-`4`/all-`5` claim, landing-quality claim, or improvement-movement claim without the complete governing `E.21` result. | Ensures one shared triage floor without turning every review into a full audit or substitute quality measurement. |
...
| **CC-E19-22 (EntityOfConcern and precision-restoration triage is applied).** | When a change touches EntityOfConcern wording, source aliases, same-referent preservation, slot/reference migration, alignment paths, role/method/work boundaries, description/publication-use guards, semio-bias repair, phrase apparatus, architecture or package rationale, quality/projection evidence, or repeated boundary doctrine, recover the pattern's own `EntityOfConcern`, first useful move, positive subject and action guidance or ordered first-use actions, practical delta, bounded non-use, source-wording and retargeting result, alignment path, ordinary references, and exact direct governors for outside claims. Use the active evaluation's own result—E.21 `PrecisionRestorationProfile` when active—and repair or report each mismatch. A grouped `E.10/E.19 passed` statement cannot replace these questions; successful questions need no E.19 recital. | Keeps precision restoration auxiliary to the pattern claim without a second evaluation record or action ontology. |
...
| **Type-correct but inert epistemic precision cleanup** | An `E.10`-triggered epistemic precision restoration removes the overread and restores kind language, but the recognition text no longer tells the reader why the distinction matters, what reader use remains, which exact subject assertion and predicate now carry the claim, or how a Plain recognition line maps back to the recovered Tech reading when both registers are used. | The review accepts typed wording while losing action guidance. | Repair the same boundary: restore a remaining admissible reader use, name the exact assertion, predicate, defining ClaimGraph and non-semantic pattern locator, repair the Tech-to-Plain mapping, or demote the phrase to reduced-use cue, quote-only wording, blocked transfer, or rewrite incomplete. |
...
| **Profile or record as reviewer.** | A selected PCP, checklist, filled findings form, result episteme, or published review record is said to have performed the review, repaired the pattern, admitted it, supplied assurance, or authorized downstream use. | Method, assignment, review/repair/verification work, applications, claims, evidence use, and decision authority collapse. | Recover each exact object and direct relation; let profiles declare checks and records package refs only. |
