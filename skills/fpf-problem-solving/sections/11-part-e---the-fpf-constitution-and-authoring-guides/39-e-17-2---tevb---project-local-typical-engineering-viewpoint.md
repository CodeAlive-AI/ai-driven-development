...
| allocation-responsibility | exact local system-role kind under A.2 and C.3.2, resolved by a context field ending in `...SystemRoleKindRef` and typed by `U.KindRef`; exact obtaining assignment occurrence under a directly declared `U.SystemRoleAssignment` species, resolved by a field ending in `...SystemRoleAssignmentRef` and typed by `U.RelationRef constrained to U.SystemRoleAssignment`; exact `SystemRoleKindRelationStructure` under A.2.7 and A.22; exact `U.Capability` under A.2.2; exact `U.Transformation` under A.3.4; an independently governed responsibility relation or selected structure when responsibility is current |
...
- **Allocation-responsibility:** holder System, local system-role kind, assignment, transformer relation, allocation, segregation, capability, and responsibility remain separate typed constraint claims unless their exact direct occurrence or selected structure is independently recovered. A local system-role kind is not an assignment; classification or assignment establishes neither responsibility nor Work; and a selected structure performs no Work.
- **Module-interface:** current `moduleIn(...)` remains a claim record. Whole-holon, candidate-module, boundary, interface, substitutability, and change-policy content stays in the coverage-rule episteme until an exact module-relation declaration supplies participant kinds, predicate, obtaining rule, and occurrence identity and current facts satisfy it. The claim record is not that relation and a module topic is not an EntityOfConcern.

Split any phrase spanning several exact subjects into separate concern epistemes, or retain it as one constraint claim over candidate content. Give each stakeholder constituent exactly one referent—exact System, local system-role kind, exact obtaining system-role assignment, C.13 collection-as-whole, or other context-local classification. Cite any responsibility concern through its separately governed direct predicate. Do not coerce heterogeneous constituents into Signatures merely to make the rows uniform.
...
| `C_allocation = {E_target.tevbHolon, E_admitted.tevbEpisteme, E_concern.systemRoleKind, E_concern.systemRoleAssignment, E_concern.systemRoleKindRelationStructure, E_concern.capability, E_concern.transformation, E_concern.responsibility, E_rule.allocationCoverage, E_rule.allocationNoWorkInference, E_rule.allocationRetargeting}` | `Q_org.allocation` is about C. Use A.22 to select `S_allocation`; exact project `P_allocation` has `EntityOfConcern=S_allocation`, is assigned local reader designator `d_allocation`, and passes E.17.0 membership before `r_allocation` is bound. | Each concern episteme depends on the target; coverage depends on all six concern epistemes and the admitted-episteme kind; no-work-inference depends on the kind, assignment, transformation, and responsibility concerns; retargeting depends on the target. | A bare *role* label, raw kind or relation reference, and raw Method are not collection members. Only the exact concern epistemes enter C. An allocation or analysis Method enters only through an exact MethodDescription episteme. The selected structure performs no Work. |
...
**Allocation-responsibility.** A conforming candidate episteme foregrounds exact Systems, local system-role kinds, obtaining assignments, relations among those kinds, capabilities, transformations, and separately governed responsibility relations or selected structures related to its holon. A label creates no classification or assignment, an assignment creates no responsibility relation, and the view itself neither assigns a System nor performs Work.
...
| allocation-responsibility | Organization and operations designers, safety or compliance readers concerned with segregation of duties, and device or system engineers | Look for the admitted holder System, exact local system-role kind and assignment occurrence, segregation and escalation constraints, capability and transformation claims, and a separately governed responsibility relation or structure. A local kind is not an assignment; neither claim proves responsibility; allocation wording is not an obtaining relation; and no view or selected structure performs the allocated Work. |
