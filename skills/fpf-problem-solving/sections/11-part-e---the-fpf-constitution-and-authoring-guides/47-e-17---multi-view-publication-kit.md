...
| Is the publication-facing unit being used as gate passage, constraint validity, adjudication, or release decision source? | Recover the exact `A.20` constraint result or `A.21` gate result and the applicable rule, subject, source results, scope, window, and action consequence. Add a log, replay witness, freshness result, or evidence path only when the governing rule or receiving use requires it; a release claim also retains its own decision or authority basis. |
...
**Gate-display boundary.** A dashboard tile, status view, or release screen exposes a gate decision only when the exact `GateDecisionResult` is recoverable with its decision subject, bounded action, profile application, check-application references, decision, action consequence, scope, window, and recheck condition under A.21. Add evidence, currentness, replay, or audit references only when the applicable rule or receiving use requires them. Without the required result and basis, the display remains orientation or source-finding only; color, label, layout, or proximity establishes neither gate passage, release permission, nor performed Work.
...
**Measure and display overread.** Before relying on a displayed number, score, percentage, color, rank, confidence, similarity, or status, recover the exact claim and its source. Retain the characteristic or criterion, scale, scope, window, aggregation rule, calibration, or evidence basis only where the applicable pattern requires it or it changes the value's interpretation. A status or ordinal judgment does not become a measurement by display. If the basis required for the intended use is missing, keep only the supported orientation or source-finding use. Use A.10 for evidence reliance, B.3 for assurance, A.20/A.21 for an actual constraint or gate claim, and F.9 only for a separately claimed Bridge or its bounded use; F.9.1 may add a stance note about an already constituted claim.

**World-contact stop.** When a source update, revocation, policy change, holon-state change, incident, model update, environmental change, or new observation changes a premise required by a downstream use, revalidate that affected use before it continues. Refresh the source, reissue the publication, or recover a changed project-side result as that use requires. Reuse unaffected results; a changed label or edition alone does not establish changed meaning or invalidate every publication use.
...
- **Green dashboard tile.** A tile says `Model ready`. Treat the tile as the `PublicationUnit` when that tile carries the present release overread. Recover the exact A.21 gate result and the source basis required by its profile application before using the tile as gate passage. A release claim also needs its own decision or authority basis. Without those, retain source-finding and status orientation only; green color or placement supplies neither result.
...
- A green dashboard tile appears to permit release. Block that interpretation when the exact gate result, its applicable profile and required check results, or the release decision or authority basis is not recoverable. Useful publication use remains: source-finding and status orientation, followed by inspection of the missing source when release is intended.
