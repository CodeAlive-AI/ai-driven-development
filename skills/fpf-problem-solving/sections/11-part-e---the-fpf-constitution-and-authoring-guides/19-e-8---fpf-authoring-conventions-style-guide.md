...
| Claim (E.8 need) | Current or still-current practice answer | Use of source | Primary source or lineage anchor | Alignment with E.8 | Adoption status |
|---|---|---|---|---|---|
...
| A narrower contribution must change same-situation use without hiding the separate family-coverage question. | Current pattern-validation practice asks for cases and actual-use evidence; domain-specific requirements work shows that domain content can change parameters, defects, checks, and verification but reports an early proof of concept; product-line scoping separates reusable-contribution and family-boundary decisions but uses software-specific machinery. | **Current-practice comparison for contribution and family scope.** The sources support testing the action-changing difference and reopening family coverage; they do not supply an FPF specificity threshold, a universal domain grammar, or a required research method. | Riehle, Harutyunyan, and Barcomb (2025), *Pattern Discovery and Validation Using Scientific Research Methods*; Chuprina et al. (2024), *Towards an Approach to Pattern-based Domain-Specific Requirements Engineering*; Marchezan de Paula et al. (2022), *Software product line scoping: A systematic literature review*. | `E.8:4.1.3` compares the same situation at comparable effort, separates action-changing difference from warranted usefulness, and routes a material family change to `E.4.DPF.DA`; `CC-SG.19a` tests the distinction. Reopen this row if current practice changes the actual-use test, the domain-action boundary, or the family-scope decision. | **Adapt and bound.** Adopt the shared actual-use and family-recheck pressure; reject source-specific research machinery and software ontology as universal FPF rules. |
| A pattern reference needs a durable address while current and earlier published bodies remain distinguishable. | Preserve an address through rearrangement and ordinary revision; distinguish the continuing pattern from the body published in each edition; interpret a local locator together with the named framework. Identifier spelling or publication position does not decide continuity. | **Current source contribution and limit.** W3C supports persistent identifiers and separation of a series from its versions. DITA 1.1 is historical evidence for identifiers unique inside one named document. Neither source decides FPF pattern identity; FPF supplies the practical-answer continuity rule for this use. | W3C, *Data on the Web Best Practices* and *URI persistence policy*; OASIS, *DITA 1.1 identifier rules*. Full status, checked date, limits, and reopen condition are maintained in `E.4.DPF:11`. | H-3, H-10, CC-SG.20, and the anti-pattern distinguish PatternID, the continuing pattern, current and edition-specific bodies, title, Part, and position; grammar and checking never decide continuity. | **Adapt and bound.** Adopt persistence and series/version separation; reject a global registry, imported document ontology, and identity inferred from syntax. Reopen if an actual migration cannot preserve references and content continuity together. |
