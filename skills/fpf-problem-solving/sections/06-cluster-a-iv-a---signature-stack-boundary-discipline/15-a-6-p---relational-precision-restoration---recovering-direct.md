...
| claim-bearing lens-use, preservation, or loss-account episteme | If the selected representation, represented object or claim content, `LensMappingMode`, `PreservedStructure`, `LostStructure`, declared lens use, stop or return condition, EntityOfConcern, or effective reference scheme changes the claim content, name another episteme. Recheck the correspondence occurrence and any world-side claim separately. Add a grounded lens overread only when a determinate representation feature makes the exact inference live. A changed display form alone does not establish a changed lens-use or loss claim. | `C.2.1` and `C.29` |
...
Scope, `Γ_time`, viewpoint, reference scheme, witnesses, admissible use, and stop or return condition stay with the direct relation or claim that actually needs them. They are not a universal qualifier kit. Add one grounded non-admissible overread only when the selected predicate, exact source wording, or one named receiver makes that inference live. An `admissible use` sentence is not an **A** claim unless the reader can point to both the mechanism application and its runtime entry predicate.
...
| basedness or dependence on an explicit base | Name the actual dependent, base, and direct relation; apply that relation's predicate and stop when the readable assertion answers the receiving use. Add scope, time, evidence, a reusable declaration, occurrence identity, or one grounded overread only when the selected predicate or one named receiver needs it. | `A.6.6` |
...
This table is a local recovery aid. Select the row that matches the current claim and stop when its direct result answers the receiving use.

| Current claim behind service/access wording | Plain action first | Concrete rule and local return boundary |
| --- | --- | --- |
...
| Provider or consumer participation | Name the participation meaning, admitted participant, and participation predicate needed now. | Use the pattern that defines the participation relation. Add A.2 classification or A.2.1 assignment only when that separate identity matters. |
| Individual duty, recommendation-as-duty, or prohibition | State the actual bearer, direct commitment predicate, modality, referents, scope and time, constitutive rule, and instituting basis. | Use one obtaining A.2.8 `U.Commitment`; return `missing-governor` when its predicate or instituting basis is unavailable. |
...
| Permission, non-prohibition, exercise, non-violation, or conflict | State the current permission-side question, bearer or beneficiary, action specification, normative-frame edition, ClaimScope, validity window, and obtaining case facts. | Use the exact A.2.8.PER result and stop until its frame and case facts are complete. |
| Exact software or physical bearer, access point, delivery entity, or proposed physical or operational arrangement | Name the exact referent and state the claim made about it; preserve any arrangement proposed by the source through evaluation. | Use A.1/A.1.SCR only when the repaired claim depends on whether that exact entity is a `U.System`. |
| Reusable way of requesting, connecting, repairing, providing, or delivering | State the reusable way of doing. | Use one `U.Method` under A.3.1; handle its description and any dated enactment in their own rows. |
...
| Actual service provision, request handling, connection, provisioning, repair, or delivery | Recover each exact performer through A.13, then admit one dated occurrence and name its Method, extent, and containing System. | Use A.15.1 `U.Work` and the direct Work relations that obtain; add F.6 only when assignment-bound attribution is needed. |
| Capability to provide or sustain service or access | Name the holder and the capability whose currentness matters. | Use one holder-dependent `U.Capability` under A.2.2 and its direct currentness evidence. |
| Ticket, case, log, measurement, evidence, or evaluation | State the particular claim carried or supported and the decision that relies on it. | Use C.2.1 and only the measurement, evaluation, result-binding, or A.10 evidence relations needed by that claim. |
| Promise use, outcome delivery, fulfilment, or acceptance | State the exact relation claimed and its participants. | Use the applicable A.2.3, evaluation, result, delivery, or acceptance predicate and stop on its missing condition. |
...
| No current direct relation states the needed claim | Preserve the participants, write the sentence the next task needs, and name the decision that cannot proceed. | Record the A.6.RCD result `missing-governor[...]` and retain that exact unresolved sentence. |

**Four language probes.** Use each probe to select the current question, then apply only the rows selected by that question.
...
- **“Restart the service.”** Name the exact bearer to restart—for example, a deployed process, component, endpoint, or host—and the action's governor.
...
**Internet-access case.** For “We sell internet access,” first state the commercial claim needed by the decision. For a promise reading, for example: `Customer-18 may rely on PromiseContent-IA-18 for the named connectivity outcome and acceptance content`. Add provider or consumer participation, status, fulfilment, or assignment only when that claim is current. If the source proposes `InternetAccessArrangement-CA17`, preserve that entity and use A.1.SCR only when the decision depends on its systemhood. Identify any connection Method, description, WorkPlan, dated Work, or A.2.8.PER grant separately when needed. Stop after the direct claim, or use A.1.STM when the remaining question is contribution to a named project system-of-interest.

**Physical repair-shop case.** For “The repair service is delayed,” select the subject from the blocked decision. A drop-off decision names the intake point; a performer question names the workshop or equipment; a responsibility question names its direct predicate and participants or returns `missing-governor`. State promise content, an individual deadline commitment, Method, procedure description, dated repair Work, fulfilment, and acceptance only when each claim is current.

**No duplication boundary.** A.6.P performs only the recovery move; the subject patterns admit the recovered objects and relations. Use A.1.SCR when the decision depends on systemhood, A.1.STM when it depends on contribution to project use, and the exact predicate or constraint for every other recovered assertion.
...
Treat "pattern X governs/owns Y" as under-specified language. Recover exact `Y` and state what X contributes: a predicate definition, use constraint, test, method description, or citation. A pattern id may remain as a locator. Use A.6.RCD for actual formal-premise use and A.6.0 for membership in or selection by a reusable signature; attribute action, ownership, and authority through the direct relations that establish them.

Likewise, "return/route/send/exit to the pattern for the next question" is procedural shorthand unless an actual relation is current. A true stop has no receiver. If work should continue, state the condition or unresolved question and name a candidate pattern whose `Use this when` entry accepts it. Preserve a real work, control, communication, transport, source, access, mathematical, or admitted B.4.1 route relation when that is the sentence's actual subject.

Ordinary instructions may say that an agent “uses” or “applies” a pattern. Expand that wording only when the claim depends on the identity of the actor, Method, description, dated Work, result, Transformation, or transformation-flow structure. A displayed sequence may describe a Method or constrain a separately admitted transformation-flow structure; establish any Work, Transformation, or flow under its direct pattern.
