...
When the admission claim relies on performed generation or discovery, `generationOrDiscoveryWorkRef` is mandatory and names the independently identified `U.Work` occurrence; otherwise omit the Method, Work, and attribution fields. When included, all facts required by A.15.1, A.2.1, and F.6 remain recoverable, and a short projection may omit only an unused assignment identifier. The Method, Work, admission-note episteme, generated result, representation, and any publication occurrence or carrier remain different objects.
