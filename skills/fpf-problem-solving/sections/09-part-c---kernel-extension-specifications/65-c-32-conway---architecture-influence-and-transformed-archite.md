...
The dated Work is `ModuleTransitionArchitecturingWork-17 : U.Work`. It runs from `2026-07-14T09:00:00+03:00` to `2026-07-16T18:00:00+03:00` and enacts `ModuleTransitionArchitecturingMethod-v3`. A locally declared relation places that Work within `ProductFamilyEngineeringSystem-A` for the stated engineering-system boundary and Work interval. The admitted system `ProductFamilyEngineeringSystem-A : U.System` is the second participant of that relation.

Two other admitted systems jointly perform the top-level Work. The local `PlantArchitecturingSystemRoleKindDomain` contains `ManufacturingArchitectureSystemRole` and `CertificationArchitectureSystemRole`. The two-participant assignment kind `PlantArchitecturingWorkAssignment` is declared under `U.SystemRoleAssignment`. Its `HolderSystemSlot` admits a `U.System`; its local `AssignedSystemRoleKindSlot` uses `PlantArchitecturingSystemRoleKindDomain`; and its predicate says that the holder is selected to supply the named architecturing contribution to Plant-A transition Work.

`ManufacturingArchitectureAssignment-17` assigns `ManufacturingArchitectureSystemRole` to `ManufacturingArchitectureTeam-A`. `CertificationArchitectureAssignment-17` assigns `CertificationArchitectureSystemRole` to `CertificationArchitectureTeam-A`. Each assignment continues through its maximal uninterrupted predicate-true interval and covers the full Work extent.

F.6 then records that `ModuleTransitionArchitecturingWork-17` is performed under each assignment. Each named performer is the holder in its assignment: `ManufacturingArchitectureTeam-A` for the manufacturing assignment and `CertificationArchitectureTeam-A` for the certification assignment. The Plant-A relation `ArchitecturingWorkChangesModuleBoundary` separately states that this Work brings about `FieldModuleBoundaryTransformation-17`.
