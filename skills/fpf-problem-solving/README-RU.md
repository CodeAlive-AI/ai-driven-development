...
Skill работает как **agentic RAG**: retrieval-augmented generation, где поиск выполняет сам агент без внешней векторной базы и embedding pipeline. Upstream-спецификация FPF разделена на двухуровневую иерархию: 15 директорий и 338 файлов. `SKILL.md` содержит router по thinking verbs, который сопоставляет намерение пользователя с нужной секцией: поиск первой практической точки входа в FPF, применение выбранного паттерна до первого полезного результата, восстановление прямого предмета из формулировок о проекте, процессе или кейсе, определение действующей или изменяемой системы, трассировка зависимостей от внешнего использования до рекурсивных систем-создателей, различение отношений и их отдельных реализаций, объединение независимых структур потоков преобразований в управляемые сети, решения, причинность, временные рассуждения, описание и синтез архитектуры, развёртывание структуры под ограничениями, перевод структуры в повествование, повторное использование точных наборов ссылок на viewpoints или создание локальных для проекта семейств viewpoints, обнаружение ontic-кандидатов и определение их первого использования, стабильность публикаций, обновление SoTA-паков, provenance и другие задачи. Затем агент читает `_index.md`, выбирает самый узкий подраздел и загружает только его в контекст. Агент одновременно является retriever, router и reasoner.
...
| 11 | Part E — Constitution & Authoring | 58 |
