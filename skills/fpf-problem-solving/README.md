...
This skill functions as **agentic RAG** — retrieval-augmented generation driven by the agent itself, with no external vector database or embedding pipeline. The upstream FPF specification is split into a two-level hierarchy (15 directories, 339 files). SKILL.md provides a thinking-verb router that maps the user's intent to the right section, including finding the first practical entry into FPF, applying a selected pattern to its first useful result, recovering project/process/case subjects and the intended meaning of “role”, identifying the acting or changed system, tracing outside-use dependencies through recursive builders, distinguishing relations from their occurrences, transforming epistemes while keeping source, receiving subject, use claims, work, and publication distinct, checking candidate admissibility and kind identity before asserting cross-local correspondence, connecting independent transformation-flow structures into governed networks, reasoning about production-work identity and completion, decision, causality, temporal reasoning, architecture description and synthesis, constraint validity and gate decisions, narrative rendering, pattern-edition continuity and impact, reusing exact viewpoint-reference bundles or authoring project-local viewpoint families, ontic-candidate detection and first-use disposition, publication stability, SoTA-pack refresh, and provenance needs. The agent then navigates `_index.md` files to pick the narrowest sub-section and loads only that into context. The agent is the retriever, the router, and the reasoner — all in one loop.
