...
| Concept | Meaning |
|---|---|
| Code rot | Gradual increase in future change cost |
| Cyclomatic complexity | Path count; above 15 triggers review by default |
| Cohesion | Related behavior changes for related reasons |
| Interacting state | Values and effects that must be reasoned about together |
| Pure core / explicit effects | Keep decisions deterministic and boundaries observable |
| Fractal coherence | Each architectural level exposes meaningful parts |
| System complexity | Cycles, duplication, coupling, hotspots, and stale migration paths |
