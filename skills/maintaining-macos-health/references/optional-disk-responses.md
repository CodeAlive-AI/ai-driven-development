...
2. **Agent plan, <= 5% free:** ask whether Codex may analyze a bounded metadata inventory and automatically open the selection page. File paths, sizes, ages and the skill's instructions are sent to the user's Codex provider. This can consume paid usage: at most one scan call and one review continuation per incident, each limited to ten minutes, with no automatic paid retry. The scan includes the two cache roots and old installer/archive files directly inside Downloads, not a full-disk audit. Confirm the chosen model (or Codex CLI default). Submit does not permit deletion; the page asks separately after the agent's review.
...
The controller builds a bounded inventory without reading file contents or secret files. Codex receives the skill text, never-touch rules and that inventory, with shell, plugins, apps, browsers, image generation, hooks, multi-agent work and code execution features disabled. User config is ignored, web search disabled, and the sandbox is read-only. The agent can return only a summary and descriptions keyed by existing candidate IDs. It cannot add paths, change operations or authorize deletion. Unknown Downloads archives retain a warning that they may be the only copy; inspect their purpose before selecting them.
...
The page/server outlives the first agent call:
